  ...
  >
    <q-card>
      ...
      <q-card-section v-if="product.img_url">
        ...
        <!-- seção que lista as descrições do produto -->
      <q-card-section>
        <div class="text-h6">
          {{ product.name }}
        </div>
        ...
        <!-- Botão de envio do produto descrito por whatsapp -->
...
export default defineComponent({
  ...
  setup (props, { emit }) {
    ...
    const handleSendWpp = () => {
      const link = encodeURI(`https://api.whatsapp.com/send?phone=55${brand.value.phone}&text=${msg} 
      - ${props.product.name} - ${formatCurrency(props.product.price)}`)
      openURL(link)
    }
