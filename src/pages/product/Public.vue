  ...
  <q-page padding>
    ...
    <div class="row">
      ...
      <q-table
        ...
        hide-pagination
      >
      <!-- Botão de pesquisar dos produtos -->
        <template v-slot:top>
          <span class="text-h6">
            Products
          </span>
          ...
          <!-- Animação de transição de paginação -->
        <template v-slot:item="props">
          ...
          >
          <!-- Titulos e config dos presços, nos cards dos produtos -->
            ...
            <!-- config de transição paralax das imagens dos produtos -->
...
export default defineComponent({
  ...
  setup () {
    ...
    const handleListProducts = async (userId) => {
      try {
        loading.value = true
        products.value = categoryId.value ? await 
        listPublic(table, userId, 'category_id', categoryId.value) : await listPublic(table, userId)
        loading.value = false
      } catch (error) {
